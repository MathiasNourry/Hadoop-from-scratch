...
def Upload_script_SSH(Username, Remote_computers):
    ...
    try:
        ...
        if error == "":
            ...
            scp_process = Popen(
                f'scp {os.path.dirname(__file__) + "/SLAVE_v3_1.py"} {Username}@{Remote_computers}:/tmp/{Username}', 
                stderr=PIPE, stdout=PIPE,
                shell=True
                )
            scp_process.wait()
...
def Deploy(Username, Remote_computers):
    ...
    with open(os.path.dirname(__file__) + "/Available_remote_computers.txt", "w") as f:
