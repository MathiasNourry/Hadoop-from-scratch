...
def Split_construction(inputfile, number_remote_computers):
    ...
    if os.path.exists(os.path.dirname(__file__) + "/Splits"):
        rmtree(os.path.dirname(__file__) + "/Splits")

    # Creation of a new blank Split folder
    os.mkdir(os.path.dirname(__file__) + "/Splits")
    ...
    split_process = CMD_instruction(
        f'cd {os.path.dirname(__file__) + "/Splits"} && split {inputfile} Split_ --additional-suffix=.txt -d -n l/{number_remote_computers}',
        print_output=False
    )

    if split_process == "OK":
        Splits_files = os.listdir(os.path.dirname(__file__) + "/Splits")
        return Splits_files
    else:
...
def Split_broadcasting(Remote_computer, Split_file):
    ...
    scp_process = CMD_instruction(
        f'scp {os.path.dirname(__file__)}/Splits/{Split_file} {username}@{Remote_computer}:/tmp/{username}/splits',
        print_output=False
        )
...
def rcomputer_list_broadcasting(Remote_computer):
    ...
    list_broadcasting_process = CMD_instruction(
        f'scp {os.path.dirname(__file__)}/Available_remote_computers.txt {username}@{Remote_computer}:/tmp/{username}',
        print_output=False
    )
...
def REDUCE(Log_count_SM_files, Remote_computer, Number_SM_files):
    ...
    if Number_SM_files == "0":
        ...
        CMD_instruction(
            f'scp {username}@{Remote_computer}:/tmp/{username}/reduce/reduce_{Remote_computer}.txt {os.path.dirname(__file__)}/Output/Reduces',
            print_output=False
        )
...
with open(os.path.dirname(__file__) + "/Output/LOG.txt", "a") as f:
    f.write("\n****************** START WORCOUNT ******************\n\n")

with open(os.path.dirname(__file__) + "/Output/DAG.txt", "a") as f:
...
DAG_construction(
    DAG_file = open(os.path.dirname(__file__) + "/Output/DAG.txt", "a"),
...
list_files = os.listdir(f"{os.path.dirname(__file__)}/Output/Reduces")
for file in list_files:
    if file == list_files[0]:  
        Reduce_count = pd.read_csv(
            f"{os.path.dirname(__file__)}/Output/Reduces/{file}", 
