...
def Run_Wordcount_process(
    ...
    if Launch_instruction_Store_modified is not None and Launch_instruction_Store_data == "Run":
        ...
        if not os.path.exists(os.path.dirname(__file__) + f'/MAP_REDUCE_process{Version}/Output'):
            os.mkdir(os.path.dirname(__file__) + f'/MAP_REDUCE_process{Version}/Output')
            os.mkdir(os.path.dirname(__file__) + f'/MAP_REDUCE_process{Version}/Output/Reduces')

        else:
            # Purge files from Reduce
            if os.path.exists(os.path.dirname(__file__) + f'/MAP_REDUCE_process{Version}/Output/Reduces'):
                list_Reduce_files = os.listdir(os.path.dirname(__file__) + f"/MAP_REDUCE_process{Version}/Output/Reduces")
                if list_Reduce_files != 0:
                    for file in list_Reduce_files:
                        os.remove(os.path.dirname(__file__) + f"/MAP_REDUCE_process{Version}/Output/Reduces/{file}")

            # Purge log file LOG.txt
            if os.path.exists(os.path.dirname(__file__) + f'/MAP_REDUCE_process{Version}/Output/LOG.txt'):
                os.remove(os.path.dirname(__file__) + f'/MAP_REDUCE_process{Version}/Output/LOG.txt')

            # Purge DAG file DAG.txt
            if os.path.exists(os.path.dirname(__file__) + f'/MAP_REDUCE_process{Version}/Output/DAG.txt'):
                os.remove(os.path.dirname(__file__) + f'/MAP_REDUCE_process{Version}/Output/DAG.txt')
